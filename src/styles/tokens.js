/**
 * Do not edit directly, this file was auto-generated.
 */

export const ColorBrand50 = "#eff6ff";
export const ColorBrand100 = "#dbeafe";
export const ColorBrand500 = "#3b82f6";
export const ColorBrand600 = "#2563eb";
export const ColorBrand900 = "#1e3a8a";
export const ColorNeutral0 = "#ffffff";
export const ColorNeutral100 = "#f5f5f5";
export const ColorNeutral500 = "#737373";
export const ColorNeutral900 = "#171717";
export const ColorSurfaceCream = "#ebe1d3";
export const ColorSurfaceDark = "#201817";
export const ColorSemanticError = "#3b82f6";
export const ColorSemanticSuccess = "#22c55e";
export const Spacing1 = "0.25rem";
export const Spacing2 = "0.5rem";
export const Spacing3 = "0.75rem";
export const Spacing4 = "1rem";
export const Spacing6 = "1.5rem";
export const Spacing8 = "2rem";
export const Spacing12 = "3rem";
export const Spacing16 = "4rem";
export const FontFamilySans = "Instrument Sans, Inter, sans-serif";
export const FontFamilyMono = "JetBrains Mono, monospace";
export const FontSizeSm = "0.875rem";
export const FontSizeBase = "1rem";
export const FontSizeLg = "1.125rem";
export const FontSizeXl = "1.25rem";
export const FontSize2xl = "1.5rem";
export const FontWeightRegular = "400";
export const FontWeightMedium = "500";
export const FontWeightBold = "700";
